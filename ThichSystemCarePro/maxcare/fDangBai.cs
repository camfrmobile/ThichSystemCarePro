...
namespace maxcare
{
	public class fDangBai : Form
	{
		...
		private void ExcuteOneThread(int indexRow, int indexPos, bool isRunSwap, string pathLD)
		{
			...
			try
			{
				...
				else
				{
					while (true)
					{
						...
						default:
						{
							bool valueBool = setting_InteractGeneral.GetValueBool("ckbDangTuong");
							...
							bool valueBool2 = setting_InteractGeneral.GetValueBool("ckbDangNhom");
							int valueInt3 = setting_InteractGeneral.GetValueInt("nudSoLuongNhomFrom");
							int valueInt4 = setting_InteractGeneral.GetValueInt("nudSoLuongNhomTo");
							bool valueBool3 = setting_InteractGeneral.GetValueBool("ckbChiDangNhomKKD");
							int valueInt5 = setting_InteractGeneral.GetValueInt("nudKhoangCachFrom");
							int valueInt6 = setting_InteractGeneral.GetValueInt("nudKhoangCachTo");
							bool valueBool4 = setting_InteractGeneral.GetValueBool("ckbVanBan");
							bool valueBool5 = setting_InteractGeneral.GetValueBool("ckbUseBackground");
							bool valueBool6 = setting_InteractGeneral.GetValueBool("ckbAnh");
							int valueInt7 = setting_InteractGeneral.GetValueInt("nudSoAnhFrom");
							int valueInt8 = setting_InteractGeneral.GetValueInt("nudSoAnhTo");
							...
							{
								int num8 = 0;
								int num9 = rd.Next(valueInt, valueInt2 + 1);
								string html = "";
								for (int n = 0; n < num9 + 5 && num8 < num9; n++)
								{
									SetStatusAccount(indexRow, text2 + Language.GetValue("Đang") + $" Đăng tươ\u0300ng ({num8 + 1}/{num9})...", device);
									try
									{
										bool flag11 = false;
										int num10 = 0;
										do
										{
											...
											case 2:
											{
												for (int num18 = 0; num18 < 5; num18++)
												{
													...
													if (device.CheckExistText("what's on your mind?", html) || device.CheckExistText("\"post\"", html) || device.CheckExistText("post audience", html))
													{
														flag11 = true;
														break;
													}
											...
											case 3:
												html = device.GetHtml();
												if ((device.CheckExistText("choose privacy public", html) || !device.TapByText("choose privacy", html)) && !device.CheckExistText("post audience", html))
												{
													continue;
												}
												...
												{
													html = device.GetHtml();
													string text9 = device.CheckExistTextsV2(html, 0.0, "\"done\"", "\"save\"");
													if (text9 != "")
													{
														device.TapByText(text9, html);
													}
													else
													{
											...
											case 5:
												if (valueBool6)
												{
													...
													device.DelayTime(1.0);
													for (int num13 = 0; num13 < 10; device.DelayTime(1.0), num13++)
													{
														device.TapByImage("DataClick\\image\\anh");
														device.DelayTime(1.0);
														html = device.GetHtml();
													...
													if (listBoundsByText.Count >= 12)
													{
														int num14 = rd.Next(0, 5);
													...
													listBoundsByText = (from t in device.GetListBoundsByText("\"photo", html)
														orderby Guid.NewGuid()
														select t).ToList();
													if (listBoundsByText.Count > 0)
													{
														int num16 = rd.Next(valueInt7, valueInt8 + 1);
														for (int num17 = 0; num17 < listBoundsByText.Count; num17++)
														{
															if (device.TapByBounds(listBoundsByText[num17], "[0,60][320,480]"))
															{
																num16--;
																if (num16 <= 0)
																{
														...
														html = device.GetHtml();
														string text8 = device.CheckExistTextsV2(html, 0.0, "\"done\"", "\"next\"");
														if (text8 != "")
														{
															flag11 = device.TapByText(text8, html);
														}
													}
													else
													{
											...
											case 1:
												...
												device.DelayRandom(1.0, 1.5);
												if (!valueBool5 || !device.TapByText("show all background options"))
												{
													continue;
												}
												...
												html = device.GetHtml();
												listBoundsByText2 = device.GetListBoundsByText(", background", html);
												if (listBoundsByText2.Count > 2)
												{
													device.TapByBounds(listBoundsByText2.OrderBy((string t) => Guid.NewGuid()).First());
													device.DelayRandom(1.0, 1.5);
												}
												device.TapByText("minimize background options", html);
												continue;
											}
											break;
										}
							...
							SetStatusAccount(indexRow, text2 + Language.GetValue("Đang") + " que\u0301t danh sa\u0301ch nho\u0301m...", device);
							device.LoadStatusLD("Get list group");
							cellAccount4 = device.GetTokenCookie().Split('|')[1];
							list2 = GetListIdGroupForLeave(cellAccount4, "", text, typeProxy, valueBool3 ? 1 : 0, ckbDieuKienThanhVien: false, 0, ckbDieuKienTuKhoa: false, null);
							if (list2.Count == 0)
							{
								break;
							}
							int num20 = 0;
							int num21 = rd.Next(valueInt3, valueInt4 + 1);
							string html2 = "";
							string text10 = "";
							for (int num22 = 0; num22 < num21 + 5; num22++)
							{
								if (num20 >= num21)
								{
									break;
								...
								text10 = list2[0];
								list2.Remove(text10);
								SetStatusAccount(indexRow, text2 + Language.GetValue("Đang") + $" Đăng ba\u0300i nho\u0301m ({num20 + 1}/{num21})...", device);
								try
								{
									bool flag12 = false;
									int num23 = 0;
									do
									{
										...
										if (num23 != 1)
										{
											...
											case 2:
											{
												for (int num30 = 0; num30 < 5; num30++)
												{
													...
													if (device.GetListText(html2).Count == 1)
													{
														device.TapByText(device.GetListText(html2)[0], html2);
														device.DelayTime(2.0);
														html2 = device.GetHtml();
													}
													if (device.CheckExistText("group rules", html2))
													{
														device.GotoBack();
														continue;
													}
													if (!device.CheckExistText("\"post\"", html2))
													{
														device.DelayTime(1.0);
														continue;
													}
											...
											case 3:
											{
												...
												string text11 = device.CheckExistTextsV2("", 10.0, "what's on your mind?", "create a public post", "submit a public post", "create a post");
												if (text11 != "")
												{
													device.TapByText(text11);
													item2 = "";
													if (!valueBool7)
													{
														if (list.Count == 0)
														...
														item2 = list.OrderBy((string t) => Guid.NewGuid()).FirstOrDefault();
														list.Remove(item2);
														goto IL_36be;
													}
													lock (lstNoiDung)
													{
														if (lstNoiDung.Count != 0)
														{
															int index2 = rd.Next(0, lstNoiDung.Count);
															item2 = lstNoiDung[index2];
															lstNoiDung.RemoveAt(index2);
															goto IL_36be;
														}
													}
													goto IL_404a;
											...
											case 4:
												if (valueBool6)
												{
													...
													device.ExecuteCMD("shell am broadcast -a android.intent.action.MEDIA_MOUNTED -d file:///sdcard/Pictures");
													device.DelayTime(1.0);
													for (int num25 = 0; num25 < 10; device.DelayTime(1.0), num25++)
													{
														device.TapByImage("DataClick\\image\\anh");
														device.DelayTime(1.0);
														html2 = device.GetHtml();
														switch (device.CheckExistTexts(html2, 0.0, "\"allow\"", "\"enable gallery access", "\"want to upload your photos", "\"photo\""))
														{
														case 1:
															device.TapByText("\"allow\"", html2);
															continue;
														case 2:
															device.TapByText("\"enable gallery access", html2);
															continue;
														case 3:
															device.TapByText("\"want to upload your photos", html2);
															device.DelayTime(2.0);
															device.GotoBack();
															continue;
														default:
													...
													device.TapByText("show media in grid layout", html2);
													device.DelayTime(1.0);
													List<string> listBoundsByText4 = device.GetListBoundsByText("\"photo\"", html2);
													if (listBoundsByText4.Count >= 12)
													{
														int num26 = rd.Next(0, 5);
														for (int num27 = 0; num27 < num26; num27++)
														{
															if (device.ScrollAndCheckScreenNotChange(1000, 1, "[130,403][188,422]", "[176,220][267,254]", "[124,359][196,423]"))
															{
																break;
															}
															device.DelayTime(1.0);
													...
													listBoundsByText4 = (from t in device.GetListBoundsByText("\"photo\"", html2)
														orderby Guid.NewGuid()
														select t).ToList();
													if (listBoundsByText4.Count > 0)
													{
														int num28 = rd.Next(valueInt7, valueInt8 + 1);
														for (int num29 = 0; num29 < listBoundsByText4.Count; num29++)
														{
															if (device.TapByBounds(listBoundsByText4[num29], "[0,60][320,420]"))
															{
																num28--;
																if (num28 <= 0)
																{
														...
														html2 = device.GetHtml();
														string text12 = device.CheckExistTextsV2(html2, 0.0, "\"done\"", "\"next\"");
														if (text12 != "")
														{
															flag12 = device.TapByText(text12, html2);
														}
													}
													else
													{
											...
											default:
												...
												device.DelayRandom(1.0, 1.5);
												device.InputTextWithUnicode(item2);
												device.DelayRandom(1.0, 1.5);
												if (!valueBool5 || !device.TapByText("show all background options"))
												{
													break;
												...
												device.ScrollAndCheckScreenNotChange(rd.Next(200, 400), 1, "[115,429][210,460]", "[154,292][246,312]", "[110,333][159,374]");
												html2 = device.GetHtml();
												listBoundsByText3 = device.GetListBoundsByText(", background", html2);
												if (listBoundsByText3.Count > 2)
												{
													device.TapByBounds(listBoundsByText3.OrderBy((string t) => Guid.NewGuid()).First());
													device.DelayRandom(1.0, 1.5);
												}
												device.TapByText("minimize background options", html2);
												break;
											}
											continue;
										}
										while (true)
										{
											device.GotoGroupQuick(text10);
											device.DelayTime(2.0);
											num2 = CheckGoToIDGroupSuccess(device, indexRow, text2);
											if (num2 == 0)
											{
												break;
											}
											...
											if (num2 != 3)
											{
												for (int num31 = 0; num31 < 10; num31++)
												{
													if (!device.TapByImage("DataClick\\image\\createpostgroup"))
													{
														...
														if (!device.TapByText("discussion\"", html2))
														{
															if (device.CheckExistTexts(html2, 0.0, "create a public post", "submit a public post", "create a post") > 0)
															{
																string text13 = device.CheckExistTextsV2(html2, 0.0, "create a public post", "submit a public post", "create a post");
																device.TapByText(text13, html2);
																flag12 = true;
																break;
															}
															if (device.CheckExistText("what's on your mind?", html2))
															{
																flag12 = true;
																break;
															}
			...
			try
			{
				int num32 = rd.Next(setting_general.GetValueInt("nudDelayCloseDeviceFrom"), setting_general.GetValueInt("nudDelayCloseDeviceTo") + 1);
				if (num32 > 0)
				{
					int tickCount2 = Environment.TickCount;
					while ((Environment.TickCount - tickCount2) / 1000 - num32 < 0)
					{
						if (!isStop)
						{
							SetStatusAccount(indexRow, text2 + Language.GetValue("Đóng LD sau {time}s...").Replace("{time}", (num32 - (Environment.TickCount - tickCount2) / 1000).ToString()));
							MCommon.Common.DelayTime(0.5);
							continue;
						}
						SetStatusAccount(indexRow, text2 + Language.GetValue("Đã dừng!"));
		...
		private int CheckGoToIDGroupSuccess(Device device, int indexRow, string statusProxy)
		{
			int result = 0;
			string text = "";
			int num = 0;
			...
			for (int i = 0; i < 60; device.DelayTime(1.0), i++)
			{
				...
				if (device.CheckExistTexts(text, 0.0, "cover photo of group", "community") <= 0)
				{
					...
					if (iThread < num)
					{
						...
						new Thread((ThreadStart)delegate
						{
							try
							{
								...
								string text2 = requestXNet.RequestGet("https://mobile.facebook.com/groups/" + text + "/madminpanel");
		...
		private List<string> GetListIdGroupForLeave(string cookie, string useragent, string proxy, int typeProxy, int typeRoiNhom, bool ckbDieuKienThanhVien, int nudThanhVienToiDa, bool ckbDieuKienTuKhoa, List<string> lstTuKhoa)
		{
			...
			try
			{
				...
				if (jObject.ContainsKey("redirect"))
				{
					input = requestXNet.RequestGet(jObject["redirect"]!.ToString()).Replace("for (;;);", "");
		...
		private void InitializeComponent()
		{
			...
			bunifuCustomLabel1.Size = new System.Drawing.Size(297, 16);
			bunifuCustomLabel1.TabIndex = 3;
			bunifuCustomLabel1.Text = "MAX SYSTEM CARE PRO - Ti\u0301nh năng Đăng ba\u0300i";
