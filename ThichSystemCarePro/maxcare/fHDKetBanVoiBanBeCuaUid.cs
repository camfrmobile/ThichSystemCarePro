...
namespace maxcare
{
	public class fHDKetBanVoiBanBeCuaUid : Form
	{
		...
		public fHDKetBanVoiBanBeCuaUid(string id_KichBan, int type = 0, string id_HanhDong = "")
		{
			...
			this.type = type;
			if (InteractSQL.GetTuongTac("", "HDKetBanVoiBanBeCuaUid").Rows.Count == 0)
			{
		...
		private void txtComment_TextChanged(object sender, EventArgs e)
		{
			try
			{
				List<string> lst = txtUid.Lines.ToList();
				lst = MCommon.Common.RemoveEmptyItems(lst);
				lblStatus.Text = string.Format(Language.GetValue("Danh sa\u0301ch Uid ({0}):"), lst.Count.ToString());
			}
			catch
			{
			}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(maxcare.fHDKetBanVoiBanBeCuaUid));
