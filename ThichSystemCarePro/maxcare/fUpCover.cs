...
namespace maxcare
{
	public class fUpCover : Form
	{
		...
		private void ExcuteOneThread(int indexRow, int indexPos, bool isRunSwap, string pathLD)
		{
			...
			try
			{
				...
				else
				{
					while (true)
					{
						...
						default:
						{
							SetStatusAccount(indexRow, text2 + Language.GetValue("Đang") + " Up Cover...", device);
							bool flag11 = false;
							int num8 = 0;
							string html = "";
							num8 = 1;
							...
							while (true)
							{
								...
								switch (CheckGoToProfileUidSuccess(device, indexRow, text2))
								{
								case 2:
									continue;
								case 0:
								...
								while (flag11)
								{
									...
									case 2:
									{
										...
										for (int n = 0; n < 5; device.DelayTime(1.0), n++)
										{
											html = device.GetHtml();
										...
										text8 = (from t in device.GetListBoundsByText("\"photo\"", html)
											orderby Guid.NewGuid()
											select t).FirstOrDefault();
										if (!string.IsNullOrEmpty(text8))
										{
											device.TapByBounds(text8);
											device.DelayTime(1.0);
											flag11 = device.TapByText("\"save\"", "", 10);
										}
										continue;
									}
									case 3:
										device.DelayTime(3.0);
										flag11 = device.WaitForTextDisappear(60.0, "uploading your cover photo.");
										continue;
									case 1:
										break;
									}
			...
			if (num3 == 1)
			{
				text9 = GetStatusAccount(indexRow);
			}
			else if (CheckIsUidFacebook(text5) && CommonRequest.CheckLiveWall(text5).StartsWith("0|"))
			{
				SetInfoAccount(cellAccount, indexRow, "Die");
				text9 = Language.GetValue("Tài khoản Die!");
			}
			try
			{
				int num13 = rd.Next(setting_general.GetValueInt("nudDelayCloseDeviceFrom"), setting_general.GetValueInt("nudDelayCloseDeviceTo") + 1);
				if (num13 > 0)
				{
					...
					while ((Environment.TickCount - tickCount2) / 1000 - num13 < 0)
					{
						if (!isStop)
						{
							SetStatusAccount(indexRow, text2 + Language.GetValue("Đóng LD sau {time}s...").Replace("{time}", (num13 - (Environment.TickCount - tickCount2) / 1000).ToString()));
							MCommon.Common.DelayTime(0.5);
							continue;
						}
						SetStatusAccount(indexRow, text2 + Language.GetValue("Đã dừng!"));
			...
			if (!(text11 == ""))
			{
				SetStatusAccount(indexRow, text2 + text9 + (flag3 ? "- Lô\u0303i mơ\u0309 link" : "") + (flag2 ? "- Facebook blocked" : ""));
			}
			else
			{
			...
			{
				string text12 = "profile\\" + text4;
				string pathTo = "profile\\" + text5;
				if (!MCommon.Common.MoveFolder(text12, pathTo) && MCommon.Common.CopyFolder(text12, pathTo))
				{
					MCommon.Common.DeleteFolder(text12);
				}
			}
		...
		{
			setting_general = new JSON_Settings("configGeneral");
			setting_InteractGeneral = new JSON_Settings("configUpCover");
		...
		{
			MCommon.Common.ShowForm(new fUpCover_CauHinh());
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(maxcare.fUpCover));
			...
			bunifuCustomLabel1.Size = new System.Drawing.Size(298, 16);
			bunifuCustomLabel1.TabIndex = 3;
			bunifuCustomLabel1.Text = "MAX SYSTEM CARE PRO - Ti\u0301nh năng Up Cover";
